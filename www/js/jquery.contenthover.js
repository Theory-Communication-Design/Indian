		...
		init : function(options) {
			...
			return this.each(function(){
				...
				if ($data.length) {
					...
					var $ch_hover = $('<div>').addClass(settings.hover_class).css({ 'width':overlay_w, 'height':overlay_h, 'overflow-x':'hidden', 'overflow-y':'scroll', 'position':'absolute', 'z-index':settings.zindex-1 }).appendTo($ch_wrapper);
