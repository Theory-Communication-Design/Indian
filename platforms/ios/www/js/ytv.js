    ...
    var YTV = YTV || function(id, opts){
        ...
        var noop = function(){},
            ...
            prepare = {
                ...
                playlists: function(res,spn){
                    ...
                    if(res && res.feed){
                        ...
                        for(i=0; i<playlists.length; i++){
							...
							//if(data.title==spn){
								//console.log(spn);
								//list += '<li class="ytv-active">';
							//} else {
								
							//}
							
							if(spn!==undefined && spn==data.title){
								list += '<li class="ytv-active">';
							} else {
								//console.log("none");
								list += '<li>';	
                            ...
                            list += '<a href="" class="btnPlaylist'+i+'" data-ytv-playlist="'+(data.plid)+'">';
						...
						var lh = doc.getElementsByClassName('ytv-list-header')[0],
                        headerLink = lh.children[0],
						shit = headerLink.children[1],
						shet = shit.children[0];
						shet.setAttribute('data-ytv-playlist-toggle', 'true');
                        //headerLink.href="#";
                       // headerLink.target="";
                        //headerLink.setAttribute('data-ytv-playlist-toggle', 'true');
                        doc.getElementsByClassName('ytv-list-header')[0].innerHTML += list;
                ...
                compileList: function(data,obj){
                    if(data && data.feed){
						//alert($(data).addClass('ytv-active'));
                        ...
                        utils.ajax.get( utils.endpoints.userInfo(), function(userInfo){
                            ...
                            /*if(settings.playlist){
								...
								var title = data.feed.media$group.media$title.$t;
								if($(obj).html()==title){
									alert($(obj).html());
								}
							...
							if(settings.playlist){
                                //user.title += ' &middot; '+(data.feed.media$group.media$title.$t);
								var spnTitle = data.feed.media$group.media$title.$t;
								//console.log(spnTitle+1);
								/*if($(obj).html()==title){
									$(obj).closest('li').addClass('ytv-active');
								}*/
							  	
                            }
                            list += '<div class="ytv-list-header ">';
                                list += '<a href="videos.html">'; //'+(user.url)+'  target="_blank" //
								list += '<img src="'+(user.thumb)+'">';
								list += '<span>'+(user.title)+'<i class="ytv-arrow down" style="display:none;" ></i></span>';
                            ...
                            if(settings.browsePlaylists){
                                ...
                                utils.ajax.get( utils.endpoints.userPlaylists(),function(res){
									prepare.playlists(res,spnTitle);
								});
            ...
            action = {
                ...
                logic: {
                    ...
                    loadVideo: function(slug, autoplay){
						...
						// FETCH INFO ON LOAD//
						//var id = target.getAttribute('data-ytv')
						//alert();
						getYouTubeInfo(slug);
                ...
                endpoints: {
                    ...
                    playlistClick: function(e){
                        ...
                        if(target && target.getAttribute('data-ytv-playlist')){
							...
							//alert($(e.srcElement).closest('li').addClass('ytv-active'));
                            //target.children[1].innerHTML = 'Loading...';
                            utils.ajax.get( utils.endpoints.playlistVids(), function(res){						
                                var lh = doc.getElementsByClassName('ytv-list-header')[0];
                                lh.className = lh.className.replace(' ytv-playlist-open', '');
                                prepare.compileList(res,e.srcElement);
                            });
...
function getYouTubeInfo(_id) {
	$.ajax({
			url: "http://gdata.youtube.com/feeds/api/videos/"+_id+"?v=2&alt=json",
			dataType: "jsonp",
...
function parseresults(data) {
		...
		var authorlink= data.entry.author[0].uri.$t;
		
		//alert(head);
		//for(i=0;i<=2;i++){
			//var head = $('.ytv-list-header').children('div').children(1).children('a');
			//var head2 = head.children();
			//alert(head.html());
		//};
		$('.ytv-video').append('<div class="container"><div id="title"></div><div id="description"></div><div id="extrainfo"></div></div>');
		$('#title').html(title);
		$('#description').html('<b>Description</b>: ' + description);
		$.ajax({
			url: authorlink+"?v=2&alt=json",
			dataType:"jsonp",
			success: function(data){
				//console.log(data.entry.link[0].href);
				var lnk=data.entry.link[0].href;
				$('#extrainfo').html('<b>Author</b>: <a href="'+lnk+'" target="_blank">' + author + '</a><br/><b>Views</b>: ' + viewcount);
			}
		});
		$("#description").each( function(){
			var htp = $(this).html().replace(/\bhttp[^ ]+/ig, wrap);
		  $(this).html(htp);
		});
}
...
function wrap( str ) {
	...
	//alert(str1[0]);
	var str2 = str.replace(/\\r\\n/g, "<br/>");
	if(typeof lnk[1] != 'undefined'){
		des = "<br/>"+lnk[1];
	} else {
	    des = '';	
	}
		return '<a href="' + lnk[0] + '">' + lnk[0] + '<\/a>'+des;
